...
To the end of supporting computer experiments to characterize the
...
gradual population growth that can push the model over the epidemic
...
use of the symbols defined in Table~\ref{tsymb}.
\begin{table}[h]
  \caption{Transition rules for our stochastic SIR model}
  \label{trules}
\begin{tabular}{lll}
\hline{Event}&{($\Delta S, \Delta I, \Delta R$)}&{Rate}\\\hline
birth of a susceptible & $(1,0,0)$ & $ \begin{cases} N_0 \mu, t < t_1\\
[N_0 + \alpha (t - t_1)] \mu, \mathrm{otherwise}
\end{cases} $\\
death of a susceptible & $(-1,0,0)$ & $S \mu$\\
infection & $(-1,1,0)$ & $   $\\
death of an infective & $(0,-1,0)$ & $I \mu$\\
recovery of an infective & $(0,-1)$ & $R \gamma$\\
