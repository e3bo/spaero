...
ans <- get_stats(x=ts, center_trend="assume_zero", lag=1, stat_bandwidth=100)
points(seq(2, length(ts)), ans$stat$autocorr)
ans <- get_stats(x=ts, center_trend="assume_zero", lag=1, stat_bandwidth=800)
points(seq(2, length(ts)), ans$stat$autocorr, col="red")
```
...
plot(autocor, type='l', xlab='time', ylab='lag-1 autocorrelation', ylim=c(0.4, 1))
ans <- get_stats(x=ts, center_trend="assume_zero", lag=1, stat_bandwidth=100)
points(seq(2, length(ts)), ans$stat$autocor)
ans <- get_stats(x=ts, center_trend="assume_zero", lag=1, stat_kernel="gaussian",
                       stat_bandwidth=100)
points(seq(2, length(ts)), ans$stat$autocor, col="red")
```
...
points(ans$stat$variance)
ans <- get_stats(x=ts, center_trend="assume_zero", stat_kernel="gaussian",
                stat_bandwidth=100, stat_trend="local_linear")
points(ans$stat$variance, col=2)
```
