...
The increasing trends in rise in the autocorrelation and variance are
typical of what happens before a critical transition. Readers
interested in this type of analyis may also want to consider
performing it with the \texttt{generic\_ews} function in the earlywarnings
package [@dakos_methods_2012]. We'll next review the input parameters
and implementation of get_stats and make notes of some differences
between it and \texttt{generic\_ews}.
...
sample moments. To clarify, the estimate of the moment for the
moving window centered on index $i$ is \begin{equation} m_i(f_j(x)) =
\sum_j w_{ij} f_j(x) / N_i, \end{equation} where $w_{ij}$ is a kernel
weight, $f_j(x)$ is the value of the moment at index $j$, and
$N_i = \sum_j w_{ij}$ is a normalization constant. Table~\ref{tstats}
provides the formulas for the statistics estimated in terms of these
moment estimates.
\begin{table}[h]
...
$\textrm{correlation time}_i$ & $-\textrm{lag} / (\log \min(\max(\textrm{autocorrelation}_i, 0), 1)$) \\
